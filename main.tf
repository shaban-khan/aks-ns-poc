...
#     }
#   }
#   response_export_values = ["*"] // Export all response properties as Terraform attributes
# }
...
#       adoptionPolicy = "Always" // Action if Kubernetes namespace with same name already exists. Possible values: 'Always', 'IfIdentical', 'Never'
#       deletePolicy   = "Delete" //Delete options of a namespace. Possible values: 'Delete' 'Keep'

#       defaultResourceQuota = {
#         cpuLimit      = each.value.defaultResourceQuota.cpuLimit
#         cpuRequest    = each.value.defaultResourceQuota.cpuRequest
#         memoryLimit   = each.value.defaultResourceQuota.memoryLimit
#         memoryRequest = each.value.defaultResourceQuota.memoryRequest
#       }

#       defaultNetworkPolicy = {
#         ingress = each.value.defaultNetworkPolicy.ingress
#         egress  = each.value.defaultNetworkPolicy.egress
#       }
#     }
#   }
...
# resource "azurerm_role_assignment" "rbac_reader_assignment" {
#   for_each = azapi_resource.managed_namespace
